...
def latest():
    try:
        ...
        # stats
        cur.execute("""
            SELECT temperature FROM weather ORDER BY id DESC LIMIT 24
        """)
        ...
        # trend
        cur.execute("""
            SELECT temperature FROM weather ORDER BY id DESC LIMIT 5
        """)
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)            temperature, humidity, rain_value, rain_status,
            wind_speed, wind_direction, visibility, alert
        ))
